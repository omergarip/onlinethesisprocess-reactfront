...
class NewReview extends Component {
    ...
    clickSubmit = event => {
        ...
        if (this.isValid()) {
            ...
            const review = {
                adviserId, title, body, references
            };
            ...
            <ul class="list">
                {
                    ...
                    <li class="task">[{ i + 1  }] { reference }<span id={ reference } onClick={this.removeCitation}>X</span></li>
                    
                ))}
                
            </ul>
            <section class="citationForm">
                <input onChange={this.handleChange("reference")} type="text" id="todoInput"
                       onKeyPress={ this.addCitation } placeholder="Please write references here and press enter!"
                />
            </section>
            <button
