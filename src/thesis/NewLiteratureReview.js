...
import { getReviewByProcessId, createReview } from './apiThesis';
import { updateReview, getProcess } from '../process/apiProcess';
import { Redirect } from 'react-router-dom';
import CKEditor from '@ckeditor/ckeditor5-react';
import ClassicEditor from '@ckeditor/ckeditor5-build-classic';

class NewLiteratureReview extends Component {
	constructor() {
		super();
		this.state = {
	...
	clickSubmit = event => {
		...
		const review = {
			processId, adviserId, reviewTitle, reviewBody, references
		}
		createReview(processId, token, review).then(data => {
			if (data.error) {
				this.setState({ error: data.error });
			} else {
				getReviewByProcessId(processId, token).then(data => {
					if (data.error) {
						this.setState({ error: data.error });
					} else {
						const reviewId = data[0]._id
						this.setState({ introId: data[0] })
						updateReview(processId, reviewId, token).then(data => {
							if (data.error) {
								this.setState({ error: data.error });
							} else {
								this.setState({
	...
	NewLiteratureReview = (reviewTitle, reviewBody) => (
		<form>
	...
	render() {
		...
		return (
			<div className="container" id="thesis">
				...
				{this.NewLiteratureReview(reviewTitle, reviewBody)}
			</div>
...
export default NewLiteratureReview;